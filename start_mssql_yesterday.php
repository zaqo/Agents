...
END;
		...
		//echo "<b> $yes_date </b>";
		// Top of the table
		echo "<table><caption><b>Суточный график работы $yes_date</b></caption>";
		...
		{ 
			...
			if ($row[0]==0){ //row for the given flight
				...
				$textsql='SELECT  oneregister.agent,agents.name FROM oneregister LEFT JOIN agents ON oneregister.agent=agents.tab_num WHERE route="'.$flightcode.'" AND date="'.$sql_date.'" ';// AND DATE_FORMAT(date,"%d-%m-%Y") = "'.$datestr.'")'; 
