...
END;
	...
	$query_day2 = "SELECT registry.route, agents.name FROM registry LEFT JOIN agents ON registry.agent2=agents.tab_num WHERE registry.date='$date_my'";
	$query_day3 = "SELECT registry.route, agents.name FROM registry LEFT JOIN agents ON registry.agent3=agents.tab_num WHERE registry.date='$date_my'";

	$day_answ = mysql_query($query_day);
	$day_answ2 = mysql_query($query_day2);
	$day_answ3 = mysql_query($query_day3);
	...
	//echo "RESPONSE is: ".$rowsin."<\br>";	
		...
		for ($j=0; $j<$rowsin; $j++)
		{
			...
			$day_row= mysql_fetch_row($day_answ);
			$day_row2= mysql_fetch_row($day_answ2);
			$day_row3= mysql_fetch_row($day_answ3);
			$ag_name=$day_row[1]+$day_row2[1]+$day_row3[1];
			$airflight=$day_row[0];
